...
class NoTTLSSTableNamesIterator extends AbstractIterator<OnDiskAtom> implements OnDiskAtomIterator
{
    ...
    public NoTTLSSTableNamesIterator(NoTTLReader sstable, DecoratedKey key, SortedSet<CellName> columns)
    {
        assert columns != null;
        ...
        RowIndexEntry indexEntry = sstable.getPosition(key, NoTTLReader.Operator.EQ);
        if (indexEntry == null)
            return;
    ...
    public NoTTLSSTableNamesIterator(NoTTLReader sstable, FileDataInput file, DecoratedKey key, SortedSet<CellName> columns, RowIndexEntry indexEntry)
    {
        assert columns != null;
        this.sstable = sstable;
        this.columns = columns;
    ...
    private void read(NoTTLReader sstable, FileDataInput file, RowIndexEntry indexEntry)
    throws IOException
    {
