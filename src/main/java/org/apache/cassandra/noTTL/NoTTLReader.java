...
public class NoTTLReader extends SSTable implements SelfRefCounted<NoTTLReader>
{
    private static final Logger logger = LoggerFactory.getLogger(NoTTLReader.class);
    ...
    public static final Comparator<NoTTLReader> maxTimestampComparator = new Comparator<NoTTLReader>()
    {
        public int compare(NoTTLReader o1, NoTTLReader o2)
        {
    ...
    public static final Comparator<NoTTLReader> sstableComparator = new Comparator<NoTTLReader>()
    {
        public int compare(NoTTLReader o1, NoTTLReader o2)
        {
    ...
    public static final Ordering<NoTTLReader> sstableOrdering = Ordering.from(sstableComparator);
    ...
    private final Ref<NoTTLReader> selfRef = new Ref<>(this, tidy);
    ...
    public static long getApproximateKeyCount(Collection<NoTTLReader> sstables)
    {
        ...
        boolean cardinalityAvailable = !sstables.isEmpty() && Iterators.all(sstables.iterator(), new Predicate<NoTTLReader>()
        {
     ...
     */
    public static double estimateCompactionGain(Set<NoTTLReader> overlapping)
    {
        Set<ICardinality> cardinalities = new HashSet<>(overlapping.size());
        for (NoTTLReader sstable : overlapping)
        {
            try
            {
    ...
    public static NoTTLReader open(Descriptor descriptor) throws IOException
    {
        CFMetaData metadata;
    ...
    public static NoTTLReader open(Descriptor desc, CFMetaData metadata) throws IOException
    {
    ...
    public static NoTTLReader open(Descriptor descriptor, Set<Component> components, CFMetaData metadata, IPartitioner partitioner) throws IOException
    {
        return open(descriptor, components, metadata, partitioner, true, true);
    }

    // use only for offline or "Standalone" operations
    public static NoTTLReader openNoValidation(Descriptor descriptor, Set<Component> components, ColumnFamilyStore cfs) throws IOException
    {
     ...
     * @param metadata
     * @param partitioner
     * @return opened NoTTLReader
     * @throws IOException
     */
    public static NoTTLReader openForBatch(Descriptor descriptor, Set<Component> components, CFMetaData metadata, IPartitioner partitioner) throws IOException
    {
        ...
        NoTTLReader sstable = internalOpen(descriptor, components, metadata, partitioner, System.currentTimeMillis(),
    ...
    public static NoTTLReader open(Descriptor descriptor,
                                     Set<Component> components,
                                     CFMetaData metadata,
                                     IPartitioner partitioner,
                                     boolean validate,  //true
                                     boolean trackHotness) throws IOException //true
    {
        // Minimum components without which we can't do anything
        assert components.contains(Component.DATA) : "Data component is missing for sstable " + descriptor;
        ...
        try
        {
            ...
            sstable.load(validationMetadata); //create dfile,ifile,bf
    ...
    public static Collection<NoTTLReader> openAll(Set<Map.Entry<Descriptor, Set<Component>>> entries,
                                                    final CFMetaData metadata,
                                                    final IPartitioner partitioner)
    {
        final Collection<NoTTLReader> sstables = new LinkedBlockingQueue<>();
     ...
     */
    public static NoTTLReader internalOpen(Descriptor desc,
                                             Set<Component> components,
                                             CFMetaData metadata,
                                             IPartitioner partitioner,
                                             SegmentedFile ifile,
                                             ...
                                             OpenReason openReason)
    {
        ...
        NoTTLReader reader = internalOpen(desc, components, metadata, partitioner, maxDataAge, sstableMetadata, openReason);
    ...
    private static NoTTLReader internalOpen(final Descriptor descriptor,
                                              Set<Component> components,
                                              CFMetaData metadata,
                                              IPartitioner partitioner,
                                              Long maxDataAge,
                                              ...
                                              OpenReason openReason)
    {
        ...
        return new NoTTLReader(descriptor, components, metadata, partitioner, maxDataAge, sstableMetadata, openReason);
    }

    protected NoTTLReader(final Descriptor desc,
                            Set<Component> components,
                            CFMetaData metadata,
                            IPartitioner partitioner,
                            long maxDataAge,
    ...
    public static long getTotalBytes(Iterable<NoTTLReader> sstables)
    {
        long sum = 0;
        for (NoTTLReader sstable : sstables)
    ...
    public static long getTotalUncompressedBytes(Iterable<NoTTLReader> sstables)
    {
        long sum = 0;
        for (NoTTLReader sstable : sstables)
    ...
    {
        return that instanceof NoTTLReader && ((NoTTLReader) that).descriptor.equals(this.descriptor);
    }
    ...
    {
        //Only Entered in the last block
        ...
        else
        {
            ...
            load(false, true); //create dfile and ifile
            loadBloomFilter(); //create bf
        }
    }
    ...
    {   //recreateBloomFilter = false; saveSummaryIfCreated = true;
        try(SegmentedFile.Builder ibuilder = SegmentedFile.getBuilder(DatabaseDescriptor.getIndexAccessMode(), false);
            SegmentedFile.Builder dbuilder = SegmentedFile.getBuilder(DatabaseDescriptor.getDiskAccessMode(), compression)) //Compression=True
        {
            boolean summaryLoaded = loadSummary(ibuilder, dbuilder); // = true;
            ...
            // ONLY Entered in This Block
            if (components.contains(Component.PRIMARY_INDEX))
            {
                ifile = ibuilder.complete(descriptor.filenameFor(Component.PRIMARY_INDEX)); //读入index文件
            }

            dfile = dbuilder.complete(descriptor.filenameFor(Component.DATA)); //读入data文件
        ...
        { // Because the tidier has not been set-up yet in NoTTLReader.open(), we must release the files in case of error
            if (ifile != null)
            {
                ifile.close();
                ifile = null;
     ...
     */
    private NoTTLReader cloneAndReplace(DecoratedKey newFirst, OpenReason reason)
    {
     ...
     * @return the cloned reader. That reader is set as a replacement by the method.
     */
    private NoTTLReader cloneAndReplace(DecoratedKey newFirst, OpenReason reason, IndexSummary newSummary)
    {
        NoTTLReader replacement = internalOpen(descriptor,
                                                 components,
                                                 metadata,
                                                 partitioner,
    ...
    // runOnClose must NOT be an anonymous or non-static inner class, nor must it retain a reference chain to this reader
    public NoTTLReader cloneWithNewStart(DecoratedKey newStart, final Runnable runOnClose)
    {
        synchronized (tidy.global)
        {
    ...
    /**
     * Returns a new NoTTLReader with the same properties as this NoTTLReader except that a new IndexSummary will
     * be built at the target samplingLevel.  This (original) NoTTLReader instance will be marked as replaced, have
     * its DeletingTask removed, and have its periodic read-meter sync task cancelled.
     * @param samplingLevel the desired sampling level for the index summary on the new NoTTLReader
     * @return a new NoTTLReader
     * @throws IOException
     */
    @SuppressWarnings("resource")
    public NoTTLReader cloneWithNewSummarySamplingLevel(ColumnFamilyStore parent, int samplingLevel) throws IOException
    {
        ...
        synchronized (tidy.global)
        {
            ...
            {
                throw new AssertionError("Attempted to clone NoTTLReader with the same index summary sampling level and " +
     ...
     * @see #getPosition(org.apache.cassandra.db.RowPosition, NoTTLReader.Operator, boolean)
     */
     ...
     */
    protected RowIndexEntry getPosition(RowPosition key, NoTTLReader.Operator op, boolean updateCacheAndStats, boolean permitMatchPastLast)
    {
        if (op == NoTTLReader.Operator.EQ)
        {
        ...
        if ((op == NoTTLReader.Operator.EQ || op == NoTTLReader.Operator.GE) && (key instanceof DecoratedKey))
        {
        ...
        else
        {
            ...
            skip = l <= 0 && (l < 0 || (!permitMatchPastLast && op == NoTTLReader.Operator.GT));
        }
        if (skip)
        {
            if (op == NoTTLReader.Operator.EQ && updateCacheAndStats)
        ...
        while (segments.hasNext())
        {
            ...
            try (FileDataInput in = segments.next())
            {
                ...
                while (!in.isEOF())
                {
                    ...
                    if (op == NoTTLReader.Operator.EQ && i <= effectiveInterval)
                    {
    ...
    {
        return new NoTTLSSTableNamesIterator(this, key, columns);
    }
    ...
    {
        return new NoTTLSSTableNamesIterator(this, input, key, columns, indexEntry);
    }
     ...
     *
     * When calling this function, the caller must ensure that the NoTTLReader is not referenced anywhere
    ...
    public ISSTableScanner getScanner(RateLimiter limiter) //(null)
    {
    ...
    {
        return NoTTLScanner.getScanner(this, dataRange, limiter); //(null)
    }
    ...
    public Set<Integer> getAncestors()
    {
        ...
        {
            NoTTLReader.logOpenException(descriptor, e);
            return Collections.emptySet();
        }
    }
    ...
    public static class SizeComparator implements Comparator<NoTTLReader>
    {
        public int compare(NoTTLReader o1, NoTTLReader o2)
        {
    ...
    public Ref<NoTTLReader> tryRef()
    {
        return selfRef.tryRef();
    }

    public Ref<NoTTLReader> selfRef()
    {
        return selfRef;
    }

    public Ref<NoTTLReader> ref()
    {
        return selfRef.ref();
    }
    ...
    /**
     * One instance per NoTTLReader we create. This references the type-shared tidy, which in turn references
    ...
    private static final class InstanceTidier implements Tidy
    {
        ...
        void setup(NoTTLReader reader, boolean trackHotness)
        {
            this.setup = true;
            this.bf = reader.bf;
        ...
        public void tidy()
        {
            // don't try to cleanup if the NoTTLReader was never fully constructed
            if (!setup)
                return;
    ...
    static final class DescriptorTypeTidy implements Tidy
    {
        ...
        DescriptorTypeTidy(Descriptor desc, NoTTLReader sstable)
        {
            this.desc = desc;
        ...
        void markObsolete(NoTTLReader instance, Tracker tracker)
        {
        ...
        @SuppressWarnings("resource")
        public static Ref<DescriptorTypeTidy> get(NoTTLReader sstable)
        {
    ...
    static final class GlobalTidy implements Tidy
    {
        ...
        GlobalTidy(final NoTTLReader reader)
        {
        ...
        @SuppressWarnings("resource")
        public static Ref<GlobalTidy> get(NoTTLReader sstable)
        {
